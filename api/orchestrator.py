...
from core.supervision import WuDehouSupervisor, SupervisionEvent
...
class Orchestrator:
    ...
    def __init__(self, target: str, model: str | None = None,
        ...
        self.mode = mode
        self.concurrency = min(concurrency, self._MAX_CONCURRENCY)
        self.max_retries = min(max_retries, self._MAX_RETRIES)
        self.roles = roles
        ...
        self._supervisor = WuDehouSupervisor()
        self._supervision_log: list[SupervisionEvent] = []
    ...
    @staticmethod
    def _redact_error_text(text: str, limit: int = 240) -> str:
        """Keep provider errors useful for retry decisions without leaking secrets."""
        redacted = re.sub(
    ...
    def _is_retryable_error(error: str) -> bool:
        """Return True for transient provider/proxy/network failures."""
        text = (error or "").lower()
        retry_markers = (
            "timeout",
            "temporarily",
        ...
        try:
            async with sem:
                async with session.post(
                    ...
                    if resp.status != 200:
                        body = self._redact_error_text(await resp.text())
                        suffix = f": {body}" if body else ""
                        result = {
                            "ok": False, "text": "", "ms": int(elapsed), "tok": 0,
                            "usage": {}, "err": f"HTTP {resp.status}{suffix}",
                        }
                    else:
    ...
    def _record_supervision(
        ...
        *,
        ...
        event = self._supervisor.review_result(
            stage=stage,
            agent_id=agent.agent_id,
            agent_name=agent.name,
            passed=passed,
        ...
        self._supervision_log.append(event)
        return event
    ...
    def _record_checkpoint(
        ...
        *,
        stage: str,
        progress: int,
        active_agents: list[DueDiligenceAgent],
    ) -> SupervisionEvent:
        event = self._supervisor.checkpoint(
            stage=stage,
            progress=progress,
    ...
    def _record_retry(
        ...
        *,
        ...
        event = self._supervisor.retry_notice(
            stage=stage,
            agent_id=agent.agent_id,
            agent_name=agent.name,
            attempt=attempt,
    ...
    def _record_degraded(
        ...
        *,
        ...
        event = self._supervisor.degrade_notice(
            stage=stage,
            agent_id=agent.agent_id,
            agent_name=agent.name,
            attempt=attempt,
    ...
    async def orchestrate(self, output_dir: str | None = None) -> dict:
        ...
        return {
            ...
            "supervision_log": [event.to_dict() for event in self._supervision_log],
            ...
            for attempt in range(self.max_retries + 1):
                ...
                if not result.get("ok"):
                    ...
                    if attempt < self.max_retries and self._is_retryable_error(err):
                        self._record_retry(
                            ...
                            agent=agent,
                            attempt=attempt + 1,
                            error=self._redact_error_text(err),
                        )
                        logger.warning(
                            "Transient API failure for %s, retry %d/%d: %s",
                            agent.agent_id,
                            attempt + 1,
                            self.max_retries,
                            self._redact_error_text(err),
                        )
                        await asyncio.sleep(self._retry_delay(attempt))  # 指数退避封顶
                        continue
                    ...
                    self._record_degraded(
                        ...
                        agent=agent,
                        attempt=attempt + 1,
                        reason=self._redact_error_text(err) or "non-retryable failure",
                    )
                    return result
                ...
                if attempt < self.max_retries:
                    ...
                    self._record_degraded(
                        ...
                        agent=agent,
                        attempt=attempt + 1,
                        reason="; ".join(v.rule for v in violations[:4]) or "quality gate failed",
                    )
                    return result
    ...
    def _build_context(self, results: list[dict], max_chars: int = 2000) -> str:
        manager = ContextBudgetManager(max_summary_chars=max_chars)
        capsule = manager.build_capsule(results, target=self.target)
