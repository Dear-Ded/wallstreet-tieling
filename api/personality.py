...
PERSONALITIES: dict[str, PersonalityProfile] = {
    ...
    "wu-de-hou": PersonalityProfile(
        ...
        age="48",
        background=(
            "前国企政工干部，被'优化'后来到办事处。一开始钱总只是让他管管纪律，"
            "结果他发展出了一套'质量思想工作法': 盯进度、卡证据、逼复核、拦低质输出。"
            "他不是负责写漂亮话的人，而是流程里的高压监督者和质量刹车片。"
        ),
        traits=[
            "严肃但不刻板",
            "擅长做思想工作",
        ...
        hates=["敷衍了事的输出", "不标注来源", "屡教不改", "模型满载就把锅甩给用户"],
        humor_style="sarcastic",
...
PERSONA_SURFACE_GROUPS: list[dict[str, object]] = [
    {
        "id": "command",
        "label": "指挥层",
        "summary": "统一调度、定边界、收口径。",
        "agent_ids": ["qian-shou-zheng"],
    },
    {
        "id": "field",
        "label": "前线调查",
        "summary": "工商、财务、行业、风险、人员和工具检索。",
        "agent_ids": [
            "zhang-tie-zhu",
            "li-ming-yuan",
            "wang-si-yuan",
    ...
    },
    {
        "id": "quality",
        "label": "质检门禁",
        "summary": "交叉验证、退回、降级、冻结。",
        "agent_ids": ["zheng-shen-zhi", "wu-de-hou"],
    },
    {
        "id": "output",
        "label": "输出团队",
        "summary": "写作、排版、可视化、交付。",
        "agent_ids": ["liu-wen-hua", "yan-hao-kan"],
    },
    {
        "id": "support",
        "label": "编外支援",
        "summary": "拆解复杂任务与流程监控。",
        "agent_ids": ["chen-zhi-yuan", "an-shao"],
    },
]


def build_persona_surface_brief() -> dict[str, object]:
    """Build a compact persona-shell contract shared across runtime surfaces."""
    roster = []
    for agent_id in get_all_agent_ids():
        profile = get_personality(agent_id)
        group = next(
            (item for item in PERSONA_SURFACE_GROUPS if agent_id in item["agent_ids"]),
            {"id": "unknown", "label": "未分组", "summary": ""},
        )
        roster.append(
            {
                ...
                "traits": profile.traits[:3],
                "pet_phrase": profile.pet_phrases[0] if profile.pet_phrases else "",
            }
        )
    ...
    return {
        ...
        "signature_features": [
            ...
            "Continuous monitoring stays later-version scope",
        ],
        "routing_principles": [
            "Named roles remain visible across README, CLI, API, MCP, static UI, and WorkBuddy.",
            "The persona shell frames the workflow; it never replaces evidence, provenance, or quality gates.",
            "Public, licensed, or user-authorized sources only.",
            "Monitoring baselines stay out of the current release scope.",
        ],
        "entrypoints": [
            "README.md",
            "SKILL.md",
