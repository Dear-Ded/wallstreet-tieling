...
def build_capability_audit(dd_profile: dict | None, strategy: dict | None, gap_analysis: dict | None,
    ...
    graph_ok = (graph_data or {}).get("edge_count", 0) > 0
    ...
    _c("qyyjt_authorized_source", True, True, fixture=not has_live or has_auth)
    ...
    _c("relationship_graph", wired=graph_ok, audited=True)
    _c("high_value_paths", wired=graph_ok)
    _c("audit_log", True, True, tested=True, audited=True)
    _c("report_sections", True, True, tested=True, report=True)
    _c("persona_activation", True, True)
