...
The report organization layer now includes the `扒光查案式调查` lens: money
...
- Broader live/public industry, product, supply-chain, customer/supplier, and competitive-position extraction; supply-chain now has public-web extraction and a report path but still needs deeper source-specific parsers and corroboration.
...
- Broader controller/UBO source-specific live field mapping beyond GLEIF parent extraction, SEC structured key-person extraction, Wikidata board-member/owner-of extraction, and the graph-level confidence model.
...
- `npm run acceptance`: `721 passed, 9 skipped`, plugin validation passed, and Apple Inc. default one-click acceptance passed after source-resilience readiness, relationship-graph auditability, API contract visibility, and relationship-edge admission preservation.
...
- focused investigation/public-web/default-public-intel/requirements regression after adding the money/goods/people `扒光查案式调查` lens: `59 passed`
...
- packaged Codex MCP smoke now covers `development_requirements`
