...
class TestOrchestratorInit:
    ...
    def test_concurrency_and_retries_are_capped(self):
        """直接实例化编排器也会限制并发和重试次数"""
        orch = Orchestrator("测试", concurrency=999, max_retries=999)
        assert orch.concurrency == orch._MAX_CONCURRENCY
        assert orch.max_retries == orch._MAX_RETRIES

    def test_retry_delay_is_exponential_and_capped(self):
        """指数退避有上限，避免失败时长时间卡住执行链路"""
        orch = Orchestrator("测试")
        assert orch._retry_delay(0) == 2
        assert orch._retry_delay(2) == 8
        assert orch._retry_delay(99) == orch._MAX_RETRY_DELAY_SEC

    def test_capacity_error_is_retryable(self):
        """中转站模型满载属于瞬时故障，应自动重试"""
        err = "Selected model is at capacity. Please try a different model."
        assert Orchestrator._is_retryable_error(err) is True

    def test_http_429_and_5xx_are_retryable(self):
        """限流和服务端错误属于可恢复故障"""
        assert Orchestrator._is_retryable_error("HTTP 429: rate limit") is True
        assert Orchestrator._is_retryable_error("HTTP 503: temporarily unavailable") is True

    def test_non_retryable_client_error(self):
        """明确的客户端错误不做无意义重试"""
        assert Orchestrator._is_retryable_error("HTTP 400: invalid request") is False

    def test_error_text_redacts_sensitive_fields(self):
        """错误摘要保留诊断价值，但不泄露密钥字段"""
        text = "token=abc123 cookie:sessionid api_key='example-secret-value' normal"
        redacted = Orchestrator._redact_error_text(text)
...
class TestWuDehouSupervisor:
    def test_supervision_reject_event_is_recorded(self, base_orch, sample_agent):
        violations = [
            Violation(
                rule="no_source",
                field="full_text",
        ...
        event = base_orch._record_supervision(
            stage="phase:1",
            agent=sample_agent,
            passed=False,
            attempt=1,
        ...
        assert event.event_type == "quality_reject"
        assert event.severity == "ERROR"
        assert "no_source" in event.violations
        assert "张铁柱" in event.message
        assert base_orch._supervision_log[-1] is event

    def test_supervision_pass_event_is_recorded(self, base_orch, sample_agent):
        event = base_orch._record_supervision(
            stage="phase:1",
            agent=sample_agent,
            passed=True,
        ...
        assert event.event_type == "quality_pass"
        assert event.severity == "INFO"
        assert "证据链" in event.message

    def test_checkpoint_message_has_high_pressure_tone(self, base_orch, sample_agent):
        event = base_orch._supervisor.checkpoint(
            stage="phase:1",
            progress=50,
            active_agents=[sample_agent],
        )

        assert event.event_type == "checkpoint"
        assert "中场检查" in event.message
        assert "张铁柱" in event.message

    def test_retry_notice_is_structured(self, base_orch, sample_agent):
        event = base_orch._supervisor.retry_notice(
            stage="phase:1",
        ...
        assert "capacity" in event.message

    def test_degrade_notice_is_structured(self, base_orch, sample_agent):
        event = base_orch._supervisor.degrade_notice(
            stage="phase:2",
            agent_id=sample_agent.agent_id,
            agent_name=sample_agent.name,
            attempt=3,
        ...
        assert event.event_type == "degraded"
        assert event.severity == "ERROR"
        assert "降级处理" in event.message

    def test_checkpoint_can_be_recorded_into_log(self, base_orch, sample_agent):
        event = base_orch._record_checkpoint(
            stage="phase:1:start",
            progress=0,
            active_agents=[sample_agent],
        )
...
class TestRunPhaseSupervisionFlow:
    @pytest.mark.asyncio
    async def test_retryable_api_failure_retries_then_passes(self, base_orch, sample_agent, monkeypatch):
        """真实 Phase 执行中，瞬时 API 故障会记录 retry 并继续推进"""
        calls = []
        good_text = (
            "注册资本1000万元[来源: tyc-mcp, 参数: company_name='测试科技', "
            "时间: 2026-06-10]。公司经营正常，财务稳健。"
        ...
        async def fake_api_call(session, sem, agent, config_dict, system_prompt):
            calls.append(agent.agent_id)
            if len(calls) == 1:
                return {"ok": False, "text": "", "ms": 1, "tok": 0, "err": "HTTP 503: at capacity"}
            return {"ok": True, "text": good_text, "ms": 1, "tok": 10, "usage": {}, "err": ""}

        monkeypatch.setattr(base_orch, "_api_call", fake_api_call)
        monkeypatch.setattr(base_orch, "_retry_delay", lambda attempt: 0)

        results = await base_orch._run_phase(
            session=None,
        ...
        event_types = [event.event_type for event in base_orch._supervision_log]
        assert "retry" in event_types
        assert "quality_pass" in event_types

    @pytest.mark.asyncio
    async def test_retry_exhaustion_records_degraded_event(self, sample_agent, monkeypatch):
        """重试耗尽后不甩锅停摆，而是降级并记录监督事件"""
        orch = Orchestrator("测试科技", max_retries=1)

        async def fake_api_call(session, sem, agent, config_dict, system_prompt):
            return {"ok": False, "text": "", "ms": 1, "tok": 0, "err": "HTTP 503: at capacity"}

        monkeypatch.setattr(orch, "_api_call", fake_api_call)
        monkeypatch.setattr(orch, "_retry_delay", lambda attempt: 0)

        results = await orch._run_phase(
            session=None,
            sem=asyncio.Semaphore(1),
            agents=[sample_agent],
            system_prompt="system",
        ...
        event_types = [event.event_type for event in orch._supervision_log]
        assert event_types.count("retry") == 1
        assert "degraded" in event_types
