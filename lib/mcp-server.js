...
const companyInputSchema = {
  ...
  properties: {
    ...
    export_dir: {
      type: 'string',
      description: 'Optional directory for DOCX, portable HTML, Markdown, JSON packet, agent-handoff, and manifest outputs.'
    },
...
const TOOLS = [
  ...
  {
    name: 'retrieval_plan',
    description: 'Build an evidence-first investigative retrieval plan from a single company subject before a full run. Returns seed company, graph, and bounded source/task plan.',
    inputSchema: {
      type: 'object',
      properties: {
        company_name: {
        ...
        limit: {
          type: 'number',
          description: 'Limit returned retrieval tasks. 0 returns all tasks.',
          minimum: 0,
          maximum: 200,
          default: 0
        }
  ...
  {
    name: 'agent_delivery_packet',
    description: 'Return the single desktop-agent delivery packet: start sequence, host adapters, verification gates, preserved fields, current-release boundaries, and submission open items.',
    inputSchema: {
      type: 'object',
      properties: {
        host_id: {
          type: 'string',
          description: 'Optional host id such as codex, claude_code, hermes, doubao_office_task_mode, open_claude_agents, workbuddy_expert_team, or universal.'
        }
      }
    }
  },
  {
    name: 'agent_delivery_doctor',
    description: 'Run the desktop-agent alpha install self-check and return pass/fail JSON for release decision, tools, package files, commands, and next actions.',
    inputSchema: {
      type: 'object',
      properties: {
        host_id: {
  ...
  {
    name: 'report_delivery_targets',
    description: 'Return required report-output targets for JSON packets, Markdown, portable HTML, DOCX red-head print packages, directory bundles, and 13-role persona interaction.',
    inputSchema: {
      type: 'object',
      properties: {}
    }
  ...
  },
  {
    ...
    description: 'Check deep-mode source configuration, authorized-source readiness, fallback policy, and no-mid-run-prompt contract before investigation.',
    inputSchema: {
      type: 'object',
      properties: {
        config: {
          type: 'string',
          description: 'Optional config path for workspace-level source configuration.'
        }
      }
    }
  },
  ...
  {
    name: 'verify_report_bundle',
    description: 'Verify a report export directory or report-export-manifest.json path: file_manifest sizes, sha256 rows, and agent-handoff decision/routing schema.',
    inputSchema: {
      type: 'object',
      properties: {
        path: {
          type: 'string',
          description: 'Report export directory or report-export-manifest.json path.'
        },
        report_dir: {
          type: 'string',
          description: 'Alias of path for directory-oriented callers.'
        }
      },
      required: ['path']
    }
  ...
  switch (name) {
    ...
    case 'source_preflight':
      return textContent(await runPythonJson(['-c', [
        ...
        `print(json.dumps(build_source_preflight(${JSON.stringify(String(args.config || '') || null)}), ensure_ascii=False))`
      ].join('; ')]));
    case 'retrieval_plan':
      return textContent(await runRetrievalPlan(args));
    ...
    case 'agent_delivery_packet':
      return textContent(await runPythonJson(['-c', [
        'import json',
        'from core.agent_delivery_packet import build_agent_delivery_packet',
        `print(json.dumps(build_agent_delivery_packet(${JSON.stringify(String(args.host_id || '') || null)}), ensure_ascii=False))`
      ].join('; ')]));
    case 'agent_delivery_doctor':
      return textContent(await runPythonJson(['-c', [
        'import json',
        'from core.agent_delivery_doctor import build_agent_delivery_doctor',
        `print(json.dumps(build_agent_delivery_doctor(${JSON.stringify(String(args.host_id || '') || null)}), ensure_ascii=False))`
      ].join('; ')]));
    case 'report_delivery_targets':
      return textContent(await runPythonJson(['-c', [
        'import json',
        'from core.report_delivery_targets import build_report_delivery_targets',
        'print(json.dumps(build_report_delivery_targets(), ensure_ascii=False))'
      ].join('; ')]));
    ...
    case 'verify_report_bundle':
      return textContent(await runVerifyReportBundle(args));
    default:
      throw new Error(`Unknown tool: ${name}`);
  }
});

async function runVerifyReportBundle(args) {
  const target = String(args.path || args.report_dir || '').trim();
  if (!target) throw new Error('path/report_dir is required');
  return await runPythonJson([path.join(ROOT, 'bin', 'verify_report_bundle.py'), target]);
}
...
async function runRetrievalPlan(args) {
  ...
  const limit = Number.isFinite(rawLimit) ? Math.min(Math.max(Math.trunc(rawLimit), 0), 200) : 0;
  const argv = [path.join(ROOT, 'bin', 'retrieval_plan.py'), company];
  if (limit > 0) {
    argv.push('--limit', String(limit));
  }
  return await runPythonJson(argv);
}
...
async function runInvestigation(args) {
  ...
  if (args.export_dir) {
    argv.push('--export-dir', String(args.export_dir));
  }
