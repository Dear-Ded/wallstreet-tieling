...
**审计检查清单**（借鉴 Ballerine 留痕机制）：
- [ ] 所有数据点是否标注来源？
- [ ] 关键数据（营收、负债率、实控人）是否有≥2个来源验证？
- [ ] 是否存在与公开信息矛盾的结论？
- [ ] [待核实] 标记是否≤总数据的15%？
- [ ] 财务数据与行业均值偏差是否已标注原因？
- [ ] 尽职深度是否匹配任务等级（轻/标/深）？
- [ ] **推论检查**：是否存在"跳跃式结论"（未经过三步法）？
- [ ] **信号检查**：每个异常信号是否附带了对比基准和偏差幅度？
- [ ] **定性检查**：是否出现了"造假""掏空""壳公司"等定性词汇？→ 必须替换为信号陈述
...
> Step 0 环境探测的核心产出。周通执行，存入上下文后全局可用。

**当前平台能力标签**：
```
[Has_Connector]  YES/NO   — 是否有企查查/天眼查等Connector
[Has_MCP]        YES/NO   — 是否有MCP Server
[Has_Bash]       YES/NO   — 是否可以执行命令行/脚本
[Has_WebSearch]  YES/NO   — 是否可以联网搜索
[Has_WebFetch]   YES/NO   — 是否可以抓取网页
[Has_SkillMgr]   YES/NO   — 是否可以安装其他Skill
[Model_Tier]     HIGH/MED — 模型推理能力级别
[Context_Window] 8K/32K   — 上下文窗口大小
```

**降级路由矩阵**：
| 缺失能力 | 降级方案 | 影响 | 主动告知 |
|----------|----------|------|----------|
| 无Connector | 全部用WebSearch+WebFetch | 速度慢、数据可能不全 | ✅ "当前平台无企查查/天眼查Connector，将用网页搜索替代" |
| 无WebSearch | 纯模型知识 | 数据截止训练日期 | ✅ "无联网能力，数据截止[训练日期]，仅作参考" |
| 无WebFetch | 仅摘要搜索结果 | 无法查看详情页 | — |
| 8K窗口 | 轻量模式（精简角色+少互动） | 报告简洁 | ✅ "当前窗口较小，已自动切换轻量模式" |
| 无Bash | 无法调用Python脚本 | 开源API无法使用 | — |
