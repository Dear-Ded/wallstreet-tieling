...
**本项目是角色扮演式prompt工程，不是多Agent系统。**

| 你可能以为 | 实际 |
|-----------|------|
| 13个独立AI Agent | 13套prompt模板，同一LLM串行扮演 |
| Agent并行协作 | 同一上下文中的角色切换 |
| 智能调度引擎 | 关键词匹配+LLM语义辅助 |
| PUA自动催促 | 角色扮演文案，无实际计时 |
| 完整OSINT工具箱 | 核心工具需pip install，降级为WebSearch |
