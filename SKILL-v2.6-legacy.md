...
problem_solving_mechanism:
  ...
  problem_reporting:
    ...
    problem_levels:
      simple:
        ...
        threshold: "单个角色可以解决"
        solution: "调度1个角色协助"
        example: "数据源不可用，换一个数据源"
      
      medium:
        description: "中等问题"
        threshold: "需要2-3个角色协作"
        solution: "调度2-3个角色协作解决"
        example: "数据冲突，需要多源验证"
    ...
    meeting_flow:
      ...
      step_5:
        ...
        template: |
          💬 郑慎之（郑审计）：
          > "方案评估："
          > "方案1：可行性{高/中/低}，风险{高/中/低}"
          > "方案2：可行性{高/中/低}，风险{高/中/低}"
          > "方案3：可行性{高/中/低}，风险{高/中/低}"
          > "建议采用方案{N}，因为..."
  ...
  interaction_examples:
    ...
    medium_problem: |
      ...
      > "收到。正在检查数据源..."
      > "发现：企查查API限流，天眼查网络超时。"
      > "解决方案：使用WebSearch直接搜索工商信息。"
      
      💬 张铁柱（张调查）：
      > "收到。切换到WebSearch，问题解决。"
    
    # 复杂问题头脑风暴
    complex_problem: |
      💬 马力全（马开盒）：
      > "钱总，遇到复杂问题。"
      > "目标人物使用多个假身份，无法确定真实身份。"
      > "需要全员讨论。"
      
      💬 钱守正（钱总）：
      > "收到。各位，开会讨论。"
      > "问题：目标人物使用多个假身份，无法确定真实身份。"
      > "请各位从各自专业角度发表意见。"
      
      💬 张铁柱（张调查）：
      > "从企业尽调角度，我建议查关联企业，通过企业关系锁定身份。"
      
      💬 李明远（李财报）：
      > "从财务分析角度，我建议查资金流向，通过交易对手锁定身份。"
      
      💬 周通（技术总监）：
      > "从技术角度，我可以使用Maigret搜索3000+网站，通过用户名关联锁定身份。"
      
      💬 郑慎之（郑审计）：
      > "从数据验证角度，我建议交叉验证各假身份的信息，找出共同点。"
      
      💬 陈志远（陈工）：
      > "汇总各方意见，有以下方案："
      > "方案1：通过关联企业锁定（张铁柱）"
      > "方案2：通过资金流向锁定（李明远）"
      > "方案3：通过Maigret搜索锁定（周通）"
      > "方案4：通过交叉验证锁定（郑慎之）"
      ...
      > "方案3：可行性高，风险低"
      > "方案4：可行性高，风险低"
      > "建议采用方案3+4组合，先用Maigret搜索，再交叉验证。"
      
      💬 钱守正（钱总）：
      > "综合各方意见，决定采用方案3+4组合。"
      > "执行方案：周通用Maigret搜索，郑慎之交叉验证。"
      > "各位，开始执行！"
...
inference_mechanism:
  ...
  # 中小企业特殊推导逻辑
  sme_inference:
    description: "中小企业独有行为判断"
    
    # 企业类型识别
    enterprise_type_detection:
      - "注册资本 < 500万 → 小微企业"
      - "员工人数 < 50人 → 小微企业"
      - "成立年限 < 3年 → 初创企业"
      - "无审计报告 → 中小企业"
    ...
    sme_risk_signals:
      ...
      # 财务风险
      financial_risk:
        - "收入与税务不匹配：申报收入 vs 纳税额"
        - "员工与社保不匹配：声称员工数 vs 参保人数"
        - "资产与负债不匹配：声称资产 vs 抵押记录"
```
